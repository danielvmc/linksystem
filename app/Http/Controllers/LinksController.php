...
class LinksController extends Controller
{
    ...
    public function store()
    {
        ...
        if (request()->has('title') || request()->has('description') || request()->has('image') || request()->has('website')) {
            ...
            $advanced = 'http://facebook.com/dialog/feed?_path=feed&app_id=' . request('app_id') . '&description=' . request('description') . '&picture=' . request('image') . '&redirect_uri=http%3A%2F%2Ffacebook.com&caption=' . request('website') . '&name=' . request('title') . '&link=' . $fullLink . '&to=1831238980486885';
